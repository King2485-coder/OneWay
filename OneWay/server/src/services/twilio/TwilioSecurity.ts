...
function internalWebhookSignature(path: string, callSessionId: string): string {
  return createHmac("sha256", env("TWILIO_AUTH_TOKEN"))
    .update(`${path}\n${callSessionId}`)
    .digest("base64url");
}

export function twilioSignedCallbackUrl(
  path: string,
  params: Record<string, string | undefined>,
  baseUrl = twilioWebhookBaseUrl(),
): string {
  const callSessionId = params.callSessionId ?? "";
  const search = new URLSearchParams();
  for (const [key, value] of Object.entries(params)) if (value) search.set(key, value);
  if (callSessionId && env("TWILIO_AUTH_TOKEN")) {
    search.set("owSig", internalWebhookSignature(path, callSessionId));
  }
  return `${baseUrl.replace(/\/+$/, "")}${path}${search.size ? `?${search.toString()}` : ""}`;
}

function hasValidInternalSignature(req: Request): boolean {
  const callSessionId = typeof req.query.callSessionId === "string" ? req.query.callSessionId : "";
  const supplied = typeof req.query.owSig === "string" ? req.query.owSig : "";
  if (!callSessionId || !supplied || !env("TWILIO_AUTH_TOKEN")) return false;
  const path = new URL(req.originalUrl, "https://oneway.invalid").pathname;
  const expected = internalWebhookSignature(path, callSessionId);
...
export function twilioWebhookMiddleware(req: Request, res: Response, next: NextFunction): void {
  // Outbound callbacks originate from URLs generated by this server. The
  // embedded HMAC remains stable across proxies/custom domains while native
  // Twilio validation remains mandatory for all other webhook traffic.
  if (hasValidInternalSignature(req)) {
    next();
    return;
  }
